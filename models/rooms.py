...
class RoomTickets(models.Model):
    _name = "hospital.room.tickets"
    _description = "Ticket"
    _check_company_auto = True
    _inherit = ["mail.thread", "mail.activity.mixin"]
    ...
    partner_id = fields.Many2one(
        ...
        string="Partner")
    rooms_id = fields.Many2one(
        comodel_name="hospital.rooms",
        required=True,
        string="Rooms")
    ...
    room_diagnose_id = fields.Many2one(
        comodel_name='hospital.room.diagnose.line',
        string="diagnose lines")
    room_diagnose_ids = fields.One2many(
        comodel_name='hospital.room.diagnose.line',
        inverse_name='room_tickets_id',
        string="Diagnosis")
    room_prescription_line_id = fields.Many2one(
        comodel_name='hospital.room.prescription.line',
        string="prescription lines")
    room_prescription_line_ids = fields.One2many(
        comodel_name='hospital.room.prescription.line',
        inverse_name='room_tickets_id',
        string="Prescriptions")
    room_invoice_line_ids = fields.One2many(
        comodel_name='hospital.room.ticket.invoice.line',
        inverse_name='room_tickets_id',
    ...
    def create(self, vals):
        if not vals.get('name') or vals['name'] == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code(
                'ms_hospital.room.tickets') or _('New')
        return super(RoomTickets, self).create(vals)
...
class RoomDiagnoseLine(models.Model):
    _name = "hospital.room.diagnose.line"
    ...
    attachment_name = fields.Char(
        ...
        required=False,
        tracking=True)
    room_tickets_id = fields.Many2one(
        comodel_name="hospital.room.tickets",
        string="Ticket")
    patients_id = fields.Many2one(
        comodel_name="hospital.patients",
...
class RoomPrescriptionLine(models.Model):
    _name = "hospital.room.prescription.line"
...
class RoomTicketInvoiceLine(models.Model):
    _name = 'hospital.room.ticket.invoice.line'
    ...
    company_id = fields.Many2one(
        comodel_name='res.company',
        string='Company',
        related='room_tickets_id.company_id',
        change_default=True,
        required=False,
        readonly=True)
    currency_id = fields.Many2one(
        comodel_name='res.currency',
        string='Currency',
        related='room_tickets_id.currency_id',
        readonly=True,
        help="Used to display the currency when tracking monetary values")
